...
execute at @s positioned ^ ^1 ^2 run tag @e[dx=0,dy=0,dz=0] add ed_red_chain
execute at @s positioned ^ ^1 ^2 run playsound minecraft:entity.ender_eye.death master @a[distance=0..10]
execute at @s as @a[tag=ed_red_chain,distance=..2] run clear @s[gamemode=!creative] minecraft:warped_fungus_on_a_stick{Red_Chain:1b} 1